            ...
            <div className="flex flex-col w-2/5 gap-5 border py-5">
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 px-10">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input type="password" name="password" {...register('password', {
                            required: true,
                            pattern: /^(?=.*[A-Z])(?=.*\d)(?=.*\d)(?=.*[@$!%*?&_-])(.*){8,64}$/gm
                        })}
                               className="p-1 rounded-md border bg-background"/>
                        {errors.password?.type === 'required' &&
                            <span className="text-error">⚠ Password is required</span>}
                        {errors.password?.type === 'pattern' &&
                            <span className="text-error">⚠ Password must contain at least 8 characters, one uppercase letter, one number and one special character</span>}
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-lg">Confirm Password:</label>
                        <input type="password" name="password" {...register('cpassword', {
                            required: true,
                            validate: {equal: value => value === getValues('password')}
                        })}
                               className="p-1 rounded-md border bg-background"/>
                    ...
                    <button
                        type="submit"
                        className="my-1 p-2 bg-secondary text-text text-xl rounded-2xl hover:brightness-[.9] border">Sign
                        Up
                    </button>
                </form>
                ...
                <div className="flex flex-col px-10">
                    <button onClick={() => router.push('/login')}
                            className="my-1 p-2 bg-primary text-white text-xl rounded-2xl hover:brightness-[.9]">Log In
                    </button>
                </div>
            </div>
